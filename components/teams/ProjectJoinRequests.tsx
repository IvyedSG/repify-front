...
export default function ProjectJoinRequests({ projectId }: ProjectJoinRequestsProps) {
  ...
  useEffect(() => {
    const fetchJoinRequests = async () => {
      ...
      try {
        const response = await fetch(`${process.env.API_URL}/usuario/projects/solicitudes_project/`, {
          method: 'POST',
          headers: {
  ...
  const handleRequestAction = async (requestId: number, action: 'approve' | 'reject') => {
    ...
    const endpoint = action === 'approve' 
      ? `${process.env.API_URL}/usuario/projects/AcceptProject/`
      : `${process.env.API_URL}/usuario/projects/Denyproject/`
