...
export default function ProjectConfigPage() {
  ...
  useEffect(() => {
    const fetchProjectDetails = async () => {
      if (session?.user?.accessToken) {
        try {
          const response = await fetch(`${process.env.NEXT_SECRET_API_URL}/usuario/projects/get-project-id/`, {
            method: 'POST',
            headers: {
  ...
  const handleSave = async () => {
    if (session?.user?.accessToken && project) {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_SECRET_API_URL}/usuario/projects/update_project/`, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${session.user.accessToken}`,
            'Content-Type': 'application/json'
  ...
  const handleDelete = async () => {
    if (session?.user?.accessToken && project) {
      try {
        const response = await fetch(`${process.env.NEXT_SECRET_API_URL}/usuario/projects/delete_project/`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${session.user.accessToken}`,
            'Content-Type': 'application/json'
