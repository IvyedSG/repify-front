...
export default function FormsSection() {
  ...
  const { data: forms, error, mutate } = useSWR<Form[]>(
    session?.user?.accessToken
      ? [`${process.env.NEXT_SECRET_API_URL}/usuario/form/get_all_forms/`, session.user.accessToken as string]
      : null,
  ...
  const confirmCreateForm = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_SECRET_API_URL}/usuario/form/create_form/`, {
        method: 'POST',
        headers: {
  ...
  const confirmDeleteForm = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_SECRET_API_URL}/usuario/form/delete_form/`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${session.user.accessToken}`,
